...
\begin{titlepage}
    ...
    \vfill
    \begin{flushright}
        \textit{Developed by: A.N. Kondratyev}\\
    \end{flushright}
    {\large \today}
\end{titlepage}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{kingside castling.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{queenside castling.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{en passant.png}
...
Completed moves are output in algebraic notation (e.g., “e4” for a pawn move from e2 to e4).
...
Supported queries and commands are listed in \nameref{sec:protocol}, and also when sending the command HELP\textbackslash{r}\textbackslash{n}, a help message with all available commands is displayed.
...
\begin{itemize}
    \item \textbf{Developer of the ST-Mate chessboard:} Kondratyev Alexander Nikolaevich.\\
...
\texttt{TABLE?} & Request current sensor readings (returns 64 values). \\ \hline
\texttt{LOADTABLE} & Load calibration table from memory (returns 64 values). \\ \hline
\texttt{TRIGLV?} & Request current trigger level of sensors (default 40). \\ \hline
\texttt{TRIGLV:X} & Set sensor trigger level (X is an integer depending on calibration). \\ \hline
\texttt{ALPHA?} & Request current exponential smoothing coefficient (default 10). \\ \hline
\texttt{ALPHA:X} & Set exponential smoothing coefficient (X is an integer). \\ \hline
\multicolumn{2}{|c|}{\textbf{User Commands}} \\ \hline
\texttt{HELP} & Request list of available commands. \\ \hline
