...
\begin{enumerate}
    \item Logo.
    \item «HINT» button — when pressed, shows a suggested move for the side whose turn it is.
...
\texttt{SEED?} & Request current seed for the pseudorandom number generator. \\ \hline
\texttt{SEED:X} & Set seed for the pseudorandom number generator (X is an integer or string). \\ \hline
