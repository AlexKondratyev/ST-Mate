...
It includes the purpose and operating conditions, preparation for use, description of operations, user actions, recommendations for troubleshooting, and guidance on mastering the system.

\subsection{Scope of Application}
“ST-Mate” is designed as an intelligent chessboard for playing classical chess.
It is suitable for use in home environments, educational institutions, chess clubs, and other locations.
...
The user is advised to read this manual before operating “ST-Mate”, as well as to be familiar with the FIDE laws of chess.
...
\begin{enumerate}
    \item Logo.
    \item “HELP” button — when pressed, shows a suggested move for the side whose turn it is.
    \item “MODE” switch — has three positions corresponding to the game modes.
    \item “PRNG” button (pseudorandom number generator) — activates the mode in which the engine makes random moves (useful for training).
\end{enumerate}
On the rear side of the board (from left to right):
\begin{enumerate}
    \item RESET button — forces a microcontroller reboot.
    \item USB Type-C power connector.
\end{enumerate}

\subsection{Selecting the Game Mode}
The “MODE” switch is located on the front panel of the “ST-Mate” chessboard.

Game modes:
\begin{itemize}
    \item \textbf{Player White} — the player controls the white pieces; the controller moves black.
    \item \textbf{Player Black} — the player controls the black pieces; the controller moves white.
...
\subsection{Using the HELP Button}
The “HELP” button is located on the side panel of the “ST-Mate” chessboard.
The hint does not affect the turn order and does not automatically make a move.

\subsection{Using the PRNG Button}
The “PRNG” button is located on the side panel of the “ST-Mate” chessboard.
The PRNG button is intended to enable random move generation by the controller in modes involving the chess engine.
The PRNG mode can only be activated when the board is powered on or reset.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.3\textwidth}|p{0.6\textwidth}|}
        ...
        If the delay exceeds 10 seconds, press “HELP” to initiate a move. \\
        \hline
...
When the “HELP” button is pressed to show a recommended move, the squares involved in the suggestion are lit in blue.\\
During castling, the squares where the king moves are lit white. After the king moves, the completed move is highlighted yellow. Then the rook move is indicated with white light; after the rook is moved, the turn passes to the opponent (or the controller).\\
...
\begin{enumerate}
    \item With the “HELP” button held down when applying power or pressing the “RESET” button.
...
\begin{itemize}
    ...
    \item WS2813 — type of addressable LED strip that allows individual control of each LED.
    \item micro-Max — compact chess engine developed by H.G. Muller. Source code available at: \url{https://home.hccnet.nl/h.g.muller/umax4_8.c}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Developer of the micro-Max chess engine:} H.G. Muller.\\
          Homepage: \url{https://home.hccnet.nl/h.g.muller/}
\end{itemize}
...
\texttt{HELP:X} & Enable/disable the help button. X=0 — button inactive, X=1 — button active. \\ \hline
...
\texttt{ALPHA?} & Request current exponential smoothing coefficient (default 4). \\ \hline
\texttt{ALPHA:X} & Set exponential smoothing coefficient (X is an integer). \\ \hline
\texttt{BRIGHT:X} & Set backlight brightness (1–100). Example: \texttt{BRIGHT:75}. \\ \hline
\texttt{THEME:X} & Set colour theme (0–11). List of themes is given in Appendix B. \\ \hline
\end{longtable}
